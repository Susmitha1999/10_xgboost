...
![Ensemble Boosting – **In the above figure, it is classification case. Here, we don't have residuals, we will see correct and incorrect predictions.**](https://www.ibm.com/content/dam/connectedassets-adobe-cms/worldwide-content/creative-assets/s-migr/ul/g/fc/4f/ensemble-learning-boosting.png)
...
![Effect of Learning Rate : ***Here, if learning rate (or step size) is too big, then we may miss the optimize point which has low residual (bottom of U shape) but if learning is rate is too small it walks through function slowly and will be able to find the point, but this requires more computational resources.***](https://bradleyboehmke.github.io/HOML/10-gradient-boosting_files/figure-html/learning-rate-fig-1.png)
...
Forest Level : **Here, as the min node size increases the tree is fitting better, for small node size it fitting noise. Also, as tree depth increase, it fits better**.
...
xgb_spec <- #Specifying XgBoost as our model type, asking to tune the hyperparameters
  boost_tree(
   # Total number of boosting iterations
    trees = tune(), #tune() to look for optimizing paramter
...
xgb_grid <- grid_latin_hypercube(
  ...
  learn_rate(),
  trees(),
  size = 100  #100 different combination of all these paramters that are possible and best
)
